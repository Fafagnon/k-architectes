...
<div style="margin-bottom: 32px;">
  ...
  <h1 class="titre-section" style="margin: 16px 0 8px 0;">Créer une opportunité.</h1>
</div>

<div style="background: var(--blanc); border: var(--trait); padding: clamp(24px, 4vw, 48px); max-width: 960px;">
  <form action="{{ route('admin.opportunites.store') }}" method="post" id="opportunity-form" class="formulaire">
    @csrf

    <div class="champ @error('title') champ--erreur @enderror" style="margin-bottom: 24px;">
      <label for="title">Intitulé du poste *</label>
      <input id="title" name="title" type="text" required value="{{ old('title') }}" placeholder="ex: Architecte junior, Dessinateur-projeteur BIM, Stage...">
    ...
    <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(280px, 1fr)); gap: 20px; margin-bottom: 24px;">
      <div class="champ @error('contract_type') champ--erreur @enderror">
        ...
        <input id="contract_type" name="contract_type" type="text" required list="contrats-suggestions" value="{{ old('contract_type', 'CDI') }}">
      ...
      <div class="champ @error('location') champ--erreur @enderror">
        ...
        <input id="location" name="location" type="text" required value="{{ old('location', 'Lomé, Togo') }}">
    ...
    <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(280px, 1fr)); gap: 20px; margin-bottom: 24px;">
      ...
      <div class="champ @error('status') champ--erreur @enderror">
        ...
        <select id="status" name="status" required>
          ...
          <option value="published" {{ old('status', 'published') === 'published' ? 'selected' : '' }}>Publiée (ouverte)</option>
          <option value="closed" {{ old('status') === 'closed' ? 'selected' : '' }}>Clôturée (visible mais non postulable)</option>
        </select>
    ...
    <div class="champ @error('excerpt') champ--erreur @enderror" style="margin-bottom: 24px;">
      ...
      <textarea id="excerpt" name="excerpt" rows="3" required maxlength="500" placeholder="Courte synthèse pour la carte d'aperçu...">{{ old('excerpt') }}</textarea>
      @error('excerpt') <p class="erreur">{{ $message }}</p> @enderror
    </div>

    <div class="champ @error('body') champ--erreur @enderror" style="margin-bottom: 32px;">
      <label for="editor">Description détaillée du poste (Missions, profil recherché, conditions) *</label>
    ...
    <div style="display: flex; gap: 16px; align-items: center;">
      <button type="submit" class="bouton">Enregistrer l'offre</button>
